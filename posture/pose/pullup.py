...
def pullup_tool(request):
    ...
    with open(output_csv_file, 'w', newline='') as csvfile:
        ...
        csvwriter.writerow([
            ...
            "arm_angle_right", "arm_angle_left", "shoulder_length", "wrist_length", "Posture"
        ])
    ...
    for frame_name in min_y_result:
        ...
        if chest_point and shoulder_left and shoulder_right and elbow_right and elbow_left and wrist_left and wrist_right:
            ...
            if (shoulder_length * 1.5) <= wrist_length <= (shoulder_length * 2.3):
                posture = "올바른 자세" if (35 <= arm_angle_right <= 55) and (35 <= arm_angle_left <= 55) else "올바르지 못한 자세"
            else:
                posture = "올바르지 못한 자세"
        else:
            posture = "올바르지 못한 자세"
